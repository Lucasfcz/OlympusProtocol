  ...
  return (
    <div className="bg-surface text-fg anim-fade px-5 pt-6 pb-5">
      <header className="flex items-center justify-between">
        <p className="label-caps text-fg-muted">SEMANA 24</p>
        <p className="label-caps-lg text-[12px]">TREINOS</p>
        <span className="w-12" />
      </header>

      <h1 className="text-2xl font-bold mt-5 tracking-tight">Sua Programação</h1>
      <p className="text-xs text-fg-muted mt-1">6 treinos planejados &middot; foco em força</p>

      <ul className="mt-6 space-y-3">
        {week.map((w) => (
          <li key={w.day} className={`card p-4 flex items-center gap-4 btn-press ${w.today ? "border-gold/60" : ""}`}>
            <div className="w-12 text-center">
              <div className={`text-[10px] tracking-widest ${w.today ? "text-gold" : "text-fg-muted"}`}>{w.day}</div>
            ...
            <div className="flex-1">
              ...
              <div className="text-[11px] text-fg-muted mt-0.5">{w.focus} &middot; {w.duration}</div>
            </div>
            {w.today ? (
