  ...
  return (
    <div className="h-full flex flex-col bg-surface text-fg anim-slide">
      ...
      <section className="px-5 mt-6 shrink-0">
        ...
        <p className="text-xs text-fg-muted mt-1">{done}/{list.length} exercícios</p>
      ...
      <ul className="flex-1 overflow-y-auto olympus-scroll px-5 mt-6 space-y-3 pb-4">
        {list.map((ex, i) => (
          <li
            key={ex.id}
            className="card p-4 flex items-center gap-3 btn-press"
            onClick={() => { if (!ex.done) navigate({ to: "/serie", search: { ex: ex.id } }); }}
          >
            ...
            <div className="flex-1">
              ...
              <div className="text-xs text-fg-muted">{ex.sets} séries</div>
            </div>
            <button
              onClick={(e) => {
                e.stopPropagation();
              ...
              className={`w-7 h-7 rounded-full border flex items-center justify-center transition-colors ${
                ex.done ? "bg-gold border-gold" : "border-fg/30"
              }`}
